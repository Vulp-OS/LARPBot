...
async def check_success(interaction, guess, adjustment, result, leverage):
    ...
    randomly generated number that corresponds to the specified star/difficulty level.
    ...
    if guess == result:
        await interaction.response.send_message(
            f'Your crafting check was a critical success! Your guess was {guess}, which matched the generated Trial '
            f'value of {result} exactly!'
        )
    elif len(check_set) != 0:
        await interaction.response.send_message(
            f'Your crafting check succeeded. Your guess was {guess}. It covered an effective range of: '
            f'{min(guess_range)}-{max(guess_range)}.\n\nThe random number was {result}. The range your effective number'
            f' needed to fall inside of was: {min(success_range)} - {max(success_range)}'
        )
    else:
        await interaction.response.send_message(
            f'Your crafting check failed. Your guess was {guess}. It covered an effective range of: '
            f'{min(guess_range)}-{max(guess_range)}.\n\nThe random number was {result}. The range your effective '
            f'number needed to fall inside of was: {min(success_range)} - {max(success_range)}'
        )
    return 0
...
async def craft(interaction: discord.Interaction, trial: int, stars: int, adjustment: int, guess: int):
    ...
    try:
        ...
        await interaction.response.send_message(
            ...
            f'no typos, and you provided arguments in the format argument:value.')
    finally:
...
async def random(interaction: discord.Interaction, low: int, high: int):
    ...
    try:
        ...
        await interaction.response.send_message(
            f'Encountered an issue with the formatting of your request for a random number!\nPlease ensure there are '
            f'no typos, you only provided min:number max:number, and that there are no excess spaces')
    finally:
...
async def larphelp(interaction: discord.Interaction, help_term: str):
    try:
        if help_term == "craft":
            message = (f'To craft, use: ```/craft trial:check_difficulty_as_a_number stars:star_count '
                       f'adjustment:adjustment_number guess:your_guess```\n\tExample: ```/craft trial:3 stars:3 '
                       f'adjustment:13 guess:14```\n')
            message += (f'Use the following stat as your adjustment number for the corresponding crafting '
